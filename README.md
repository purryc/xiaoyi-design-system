...
界面右上角可切换中文 / English，语言选择保存在本机；也可使用 `?lang=en` 或 `?lang=zh`。所有 Markdown 都含完整中英文说明。原始参考图中的中文保持原样，并附英文解读。
...
- `src/i18n/`：语言切换、802 条英文翻译与 JSX 展示边界。
...
Use **中文 / English** in the header. The choice is remembered locally; `?lang=en` and `?lang=zh` also select a language. All Markdown files contain both languages. Original Chinese reference figures remain unchanged and have English explanations.
